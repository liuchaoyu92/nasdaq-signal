...
TOTAL_CAPITAL = cfg["total_capital"]           # 总资金
CURRENT_INVESTED = cfg["current_invested"]     # 已投入金额
...
def calculate_suggestion(target_pct, current_pct, total_capital):
    gap = target_pct - current_pct
    if gap <= 0.02:
        return 0, "仓位已达目标，继续观望"
    amount = round(total_capital * gap / 2)
    if amount < 500:
        return 0, "差距过小，无需操作"
    return amount, None
...
def build_html(d):
    ...
    if d["suggest_amount"] > 0:
        ...
        <div class="sug-sub">建议本次加仓（分批策略，先补一半差距）</div>"""
    else:
        suggest_html = f"""
        <div class="sug-none">{d['no_action_reason']}</div>"""
    ...
    # GitHub edit URL for config.json
    repo = os.environ.get("GITHUB_REPOSITORY", "your-username/nasdaq-pwa")
...
.hdr-t{{ font:600 11px/1 var(--fs); letter-spacing:.12em;
...
.lbl{{ font:600 10px/1 var(--fs); letter-spacing:.1em;
...
.state-val{{ font:700 24px/1 var(--fs); color:var(--state);
  display:flex; align-items:center; gap:8px; }}
.state-dot{{ width:9px;height:9px; border-radius:50%;
  background:var(--state); box-shadow:0 0 7px var(--state); flex-shrink:0; }}
...
.mv.pos{{ color:var(--normal); }}
.mv.neg{{ color:var(--severe); }}
.mv.ora{{ color:#f97316; }}
...
.upd-link{{
  ...
  padding:13px; font:600 14px/1 var(--fs);
}}
...
<div class="grid">
  <div class="metric">
    ...
    <div class="mv {'pos' if d['rsi'] < 35 else 'neg' if d['rsi'] > 65 else ''}">{d['rsi']}</div>
    <div class="msub">{d['rsi_hint']}</div>
  </div>
  <div class="metric">
    <div class="lbl">近1年回撤</div>
    <div class="mv {'pos' if d['drawdown_pct'] < -10 else 'ora' if d['drawdown_pct'] < -5 else ''}">{d['drawdown_pct']}%</div>
    <div class="msub">{d['drawdown_hint']}</div>
  </div>
  <div class="metric">
    <div class="lbl">VIX 恐慌</div>
    <div class="mv">{d['vix'] if d['vix'] else 'N/A'}</div>
...
<div class="upd-card">
  <div class="lbl">买入后更新仓位</div>
  <div class="upd-desc">点下方按钮，把 <code style="color:var(--accent)">current_invested</code> 改成你实际投入的金额，保存后约1分钟自动更新页面。</div>
  <a class="upd-link" href="{edit_url}" target="_blank">✏️ 去 GitHub 更新金额</a>
</div>

<div class="footer">数据每天北京时间 9:00 自动更新 · 仅供参考，不构成投资建议</div>
...
def main():
    ...
    suggest_amount, no_action_reason = calculate_suggestion(
        target_pct, CURRENT_POSITION_PCT, TOTAL_CAPITAL)
    ...
    print(f"✓ 生成完成：{date_str} | {state_label} | RSI {rsi} | 回撤 {drawdown}%")
